...
function Applicants() {
    ...
    return (
        <AuthLayout>
            <div className="flex p-20 space-x-7">
                <div className="w-1/4 border border-gray-300 shadow rounded-3xl">
                    {applicants.map((applicant, index) => (
                        <>
                            <div className={`hover:bg-gray-50 p-5 space-x-3 w-full cursor-pointer flex items-center`} onClick={() => setSelectedApplicant(index)}>
                                <img src={applicant.image} className="w-24 h-24 object-cover border border-gray-500 shadow rounded-full" alt={applicant.name} />
                ...
                {!isNaN(selectedApplicant) && <div className="h-full w-3/4 border border-gray-300 shadow rounded-3xl">
                    <div className={`p-5 flex space-x-7 items-center ${applicants[selectedApplicant].approval_status === 1 ? 'bg-green-100' : applicants[selectedApplicant].approval_status === 0 ? 'bg-red-100' : ''} rounded-t-3xl`}>
                        <img src={applicants[selectedApplicant].image} className="w-24 border rounded-full h-24 object-cover border-gray-500 shadow" alt={applicants[selectedApplicant].name} />
                        <div className="flex w-full items-center justify-between">
                            <h5 className="text-2xl">{applicants[selectedApplicant].name}</h5>
                    ...
                    <div className="flex p-5 space-x-7">
                        <p><strong className="font-semibold">email</strong>: {applicants[selectedApplicant].email}</p>
                        <p><strong className="font-semibold">Phone Number</strong>: {applicants[selectedApplicant].phone_number}</p>
                    </div>
                    <hr className="h-px bg-gray-100 border-0 w-full" />
                    <div className="p-10">
