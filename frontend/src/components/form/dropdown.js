...
const dropdown = ({ options, className, selectClass, icon, required=false }) => (
    <div className={`${className} relative rounded-full h-14 my-3 focus:bg-white focus:shadow-md focus:ring focus:ring-[#1C7280] w-2/3 px-5`}>
        {icon && <span>{icon}</span>}
        <select className={`${selectClass} rounded-full h-14 bg-transparent w-full text-lg duration-150 focus:outline-none appearance-none`} required={required}>
